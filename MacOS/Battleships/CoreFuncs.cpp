...
{
    srand(static_cast<unsigned int>(time(0))); //Для генерации случайных чисел с сидом на времени
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        numberOfCoord = arc4random_uniform(INT_MAX) % inputVector.size();
        ...
        while (!isSuccessful)
        {
            angleParameter = arc4random_uniform(INT_MAX) % 4;
...
void DisplayGrid(int userGrid[10][10], int generatedGrid[10][10], bool displayFullComputerGrid)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        for (int j = 0; j < 10; j++)
        {
            ...
            {
            case 4:
                colorMarker = "\033[7;49;34m"; //Синий
            ...
            case 2:
                colorMarker = "\033[43m"; //Желтый
                break;

            case 3:
                colorMarker = "\033[7;49;31m"; //Красный
                break;
...
void GenerateGrid(int (*input)[10])
{
    ...
    usleep(100); //Ждём, чтобы получать более случайные значения по сиду от времени
...
bool ComputerMove(int (*inputGrid)[10], int (*computerGrid)[10], vector<vector<int>>& inputVector, bool* moveCondition, int (*nearbyArea))
{
    ...
    x = -1; y = -1;
    srand(static_cast<unsigned int>(time(0))); //Сид по времени для получения случайных чисел
    ...
    if (x == -1 && y == -1)
    {
        numberOfCoord = arc4random_uniform(INT_MAX) % inputVector.size(); //Берём случайный номер от нулевого до последнего элемента inputVector
        vector<int> coordElement = inputVector[numberOfCoord]; //Берём вектор с координатой из общего вектора
        x = coordElement[0];
        y = coordElement[1];
    ...
    else
    {
        ...
        if (nearbyArea[0] != 2 && nearbyArea[1] != 2 && nearbyArea[2] != 2 && nearbyArea[3] != 2)
        {
            ...
            while (!isSuccessful)
            {
                int angleParameter = arc4random_uniform(INT_MAX) % 4; //Генерируем случайный угол выстрела. 0 - вверх, 3 - влево
